...
const Card: React.FC<CardProps> = ({ href, imgSrc, layoutId }) => {
  return (
    <Link href={href}>
      <div className="relative h-20 md:h-40 rounded-xl cursor-pointer hover:bg-gray-50 transition">
          <motion.img key={href} src={imgSrc} className="w-full h-full" layoutId={layoutId} />
        </div>
    </Link>
  )
}
